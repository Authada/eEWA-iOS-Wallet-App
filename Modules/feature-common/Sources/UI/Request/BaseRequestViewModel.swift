...
open class BaseRequestViewModel<Router: RouterHost>: BaseViewModel<Router, RequestViewState> {
    ...
    func checkfor(sectionID: String) -> String? {
        for item in viewState.items {
            if let datesection = item.isDataSection, datesection.id == sectionID {
                return datesection.id
            }
        }
        return nil
    }
    ...
    func onSelectionChanged(id: String) {
        ...
        var items = viewState.items.map {
            if var row = $0.isDataRow, row.id == id {
                row.setSelected(!row.isSelected)
                return RequestDataUIModel.requestDataRow(row)
            } else if var section = $0.isDataSection, section.id == id {
                section.setSelected(!section.isSelected)
                return RequestDataUIModel.requestDataSection(section)
            }
            return $0
        }
                
        let sectionID = items.filter({
            item in item.isDataSection?.id == id
        })
        
        if !sectionID.isEmpty {
            items = items.map {
                if var row = $0.isDataRow {
                    let rowIDsuffix = String(row.id.split(separator:"_").last ?? "")
                    if let section = sectionID.last, section.isDataSection?.id == rowIDsuffix {
                        row.setSelected(!(section.isDataSection?.isSelected == true))
                    }
