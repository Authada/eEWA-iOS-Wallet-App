...
public class WalletKitControllerStub:WalletKitController {
    ...
    public  func resolveOfferUrlDocTypes(uriOffer p0: String) async throws -> EudiWalletKit.OfferedIssuanceModel {
        return DefaultValueRegistry.defaultValue(for: (EudiWalletKit.OfferedIssuanceModel).self)
    }
    
    public func issueDocumentsByOfferUrl(offerUri: String, docTypes: [EudiWalletKit.OfferedDocModel], txCodeValue: String?) async throws -> [WalletStorage.Document] {
        return DefaultValueRegistry.defaultValue(for: ([WalletStorage.Document]).self)
    }
