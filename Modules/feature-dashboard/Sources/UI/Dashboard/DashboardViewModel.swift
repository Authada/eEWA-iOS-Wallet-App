...
final class DashboardViewModel<Router: RouterHost>: BaseViewModel<Router, DashboardState> {
    ...
    func onOpenRPWebdemo() {
        if let url = URL(string: "Wallet RP Webdemo URL".valueFromBundle), UIApplication.shared.canOpenURL(url) {
    ...
    func onShowProxyDetail() {
        isMoreModalShowing = false
        router.push(with: .proxyPIDDetails)
    }
    
    private func handleDeepLink() async {
        if let deepLink = deepLinkController.getPendingDeepLinkAction() {
            await deepLinkController.handleDeepLinkAction(routerHost: router, deepLinkExecutable: deepLink)
        }
    }
